...
let isInMap = (map, optionalKey: option('a)): bool =>
  switch (optionalKey) {
  | None => false
...
let getListDiff = (oldNodes: list(node), newNodes: list(node)): diff => {
  ...
  while (i^ < List.length(simulateList^)) {
    if (List.nth(simulateList^, i^) === None) {
      moves :=
        List.append(moves^, [{index: i^, moveType: Remove, item: None}]);
           ...
           | Some(nextItem) =>
             ...
             if (nextItemKey === itemKey) {
               moves :=
                 List.append(
                   moves^,
                   [{index: i, moveType: Remove, item: None}],
                 );
               ...
               moves :=
                 List.append(
                   moves^,
                   [{index: i, moveType: Insert, item: Some(item)}],
                 );
             };
           | None => 42 |> ignore
           };
